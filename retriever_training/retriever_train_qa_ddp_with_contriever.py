...
def train(args, logger: logging.Logger):
    ...
    lm_tok = None
    teacher = None
    if args.objective in {"rag", "replug"}:
    ...
    while not stop_training:
        ...
        for batch in dataset.iter_batches(batch_size=args.batch_size, epoch=epoch):
            ...
            if cfg.objective in {"rag", "replug"}:
                ...
                t2 = time.time()
                for q, docs, doc_ll in zip(batch_q, docs_per_example, doc_loglik_per_example):
                    if len(docs) == 0:
                        continue
                    ...
                    if cfg.objective == "rag":
                        loss_i = rag_sequence_marginal_nll(retr_logits / cfg.retr_temperature, doc_ll.to(device))
                        with torch.no_grad():
                        ...
                        loss_i, teacher_q, retr_p = replug_kl_loss(retr_logits, doc_ll.to(device), cfg.replug_gamma, cfg.replug_beta)
                        with torch.no_grad():
            ...
            if cfg.objective == "contriever":
                metric_denom = max(1.0, usable_examples)
            else:
                metric_denom = float(denom)
            mean_avg_retrieved = avg_retrieved / metric_denom
            mean_target_entropy = avg_target_entropy / metric_denom if cfg.objective in {"rag", "replug"} else 0.0
            mean_best_doc_rank = avg_best_doc_rank / metric_denom
            mean_gold_mass = avg_gold_mass / metric_denom
            mean_negatives = avg_negatives / metric_denom if cfg.objective == "contriever" else 0.0
            mean_top_positive_rank = (avg_top_positive_rank / max(avg_has_top_positive, 1.0)) if cfg.objective == "contriever" else 0.0
            hit_top_positive = (avg_has_top_positive / metric_denom) if cfg.objective == "contriever" else 0.0
            usable_example_rate = (usable_examples / max(1.0, examined_examples)) if cfg.objective == "contriever" else 1.0
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Distributed retriever training with switchable RAG, RePLUG, or query-only contrastive objective"
    )

    ap.add_argument("--train_jsonl", required=True, help="QA JSONL for rag/replug, or aligned contrastive JSONL for contriever")
    ap.add_argument("--faiss_index", required=True)
    ap.add_argument("--sqlite_db", required=True)

    ap.add_argument("--objective", default="rag", choices=["rag", "replug", "contriever"], help="Retriever training objective")
    ...
    if args.objective in {"rag", "replug"} and not args.lm_name:
        ap.error("--lm_name is required for objective rag/replug")
    return args
